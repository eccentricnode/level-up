import { NgModule } from '@angular/core';
import { TestBed } from '@angular/core/testing';
import { readFirst } from '@nrwl/nx/testing';

import { EffectsModule } from '@ngrx/effects';
import { StoreModule, Store } from '@ngrx/store';

import { NxModule } from '@nrwl/nx';

import { FruitsEffects } from './fruits.effects';
import { FruitsFacade } from './fruits.facade';

import { fruitsQuery } from './fruits.selectors';
import { LoadFruits, FruitsLoaded } from './fruits.actions';
import {
  FruitsState,
  Entity,
  initialState,
  fruitsReducer
} from './fruits.reducer';

interface TestSchema {
  fruits: FruitsState;
}

describe('FruitsFacade', () => {
  let facade: FruitsFacade;
  let store: Store<TestSchema>;
  let createFruits;

  beforeEach(() => {
    createFruits = (id: string, name = ''): Entity => ({
      id,
      name: name || `name-${id}`
    });
  });

  describe('used in NgModule', () => {
    beforeEach(() => {
      @NgModule({
        imports: [
          StoreModule.forFeature('fruits', fruitsReducer, { initialState }),
          EffectsModule.forFeature([FruitsEffects])
        ],
        providers: [FruitsFacade]
      })
      class CustomFeatureModule {}

      @NgModule({
        imports: [
          NxModule.forRoot(),
          StoreModule.forRoot({}),
          EffectsModule.forRoot([]),
          CustomFeatureModule
        ]
      })
      class RootModule {}
      TestBed.configureTestingModule({ imports: [RootModule] });

      store = TestBed.get(Store);
      facade = TestBed.get(FruitsFacade);
    });

    /**
     * The initially generated facade::loadAll() returns empty array
     */
    it('loadAll() should return empty list with loaded == true', async done => {
      try {
        let list = await readFirst(facade.allFruits$);
        let isLoaded = await readFirst(facade.loaded$);

        expect(list.length).toBe(0);
        expect(isLoaded).toBe(false);

        facade.loadAll();

        list = await readFirst(facade.allFruits$);
        isLoaded = await readFirst(facade.loaded$);

        expect(list.length).toBe(0);
        expect(isLoaded).toBe(true);

        done();
      } catch (err) {
        done.fail(err);
      }
    });

    /**
     * Use `FruitsLoaded` to manually submit list for state management
     */
    it('allFruits$ should return the loaded list; and loaded flag == true', async done => {
      try {
        let list = await readFirst(facade.allFruits$);
        let isLoaded = await readFirst(facade.loaded$);

        expect(list.length).toBe(0);
        expect(isLoaded).toBe(false);

        store.dispatch(
          new FruitsLoaded([createFruits('AAA'), createFruits('BBB')])
        );

        list = await readFirst(facade.allFruits$);
        isLoaded = await readFirst(facade.loaded$);

        expect(list.length).toBe(2);
        expect(isLoaded).toBe(true);

        done();
      } catch (err) {
        done.fail(err);
      }
    });
  });
});
